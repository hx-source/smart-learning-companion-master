...
这是一个基于 Flask 的智能学习助手项目，集成了用户认证、知识库管理、文档处理、向量检索和本地 Ollama 模型调用。

## 核心功能

- 用户注册 / 登录 / 注销
- 多角色权限：学生、教师、管理员
- 管理员后台：用户创建、角色修改、用户删除、班级知识库查看
- 邮箱验证码注册与密码重置
- 账户锁定与登录安全策略
- 用户资料与头像上传
- 文档上传与知识库管理
...
- 登录支持用户名或邮箱
- 用户角色分为学生、教师、管理员
- 普通用户不能自行切换角色，角色只能由管理员在后台修改
- 学生端不显示后台入口，也不能访问后台接口
- 学生端只提供个人知识库创建和维护
- 学生可以直接查看和问答老师发布的班级知识库，但不能上传、删除、重命名或删除老师班级知识库中的文件
- 教师上传文档时可明确选择“个人知识库”或“班级知识库”
- 管理员可以管理用户、角色和班级知识库
- 登录失败次数限制与锁定策略
- JWT Token 与 CSRF 保护
- 密码重置与密码修改
...
- 知识库管理页面为独立页面：`/knowledge-base`
- 创建/删除/重命名知识库
- 浏览知识库列表
- 上传文档到指定知识库
- 学生可创建个人知识库并上传自己的资料
- 教师上传资料时可选择个人知识库或班级知识库
- 教师选择班级知识库并填写班级名称时，会发布班级知识库
- 老师发布的班级知识库对学生可见、可用于问答，但学生不可修改
- 查看知识库统计信息
- 查看文件来源
- 按文件来源删除已上传文件内容
...
- 访问 `/register` 创建普通用户，或使用下方默认账号登录
- 登录后访问首页、个人中心、知识库页面
- 学生可访问 `/knowledge-base` 创建个人知识库并上传文档
- 教师或管理员上传文档时可选择“上传到个人知识库”或“发布为班级知识库”
- 学生可在知识库下拉框中选择老师发布的班级知识库进行问答，但不能修改老师发布的知识库
- 管理员可访问 `/admin` 进入后台管理用户和角色
- 在问答界面输入问题，系统会先尝试检索知识库，然后生成回答
- 问答支持流式输出

## 默认账号

以下账号用于本地开发、演示和权限测试。首次部署到真实环境后，请尽快修改默认密码。

| 角色 | 用户名 | 密码 | 邮箱 | 权限说明 |
| --- | --- | --- | --- | --- |
| 管理员 | `admin` | `Admin@2026` | `admin@zhixueban.com` | 可进入 `/admin`，管理用户、角色和班级知识库 |
| 教师 | `teacher` | `Teacher@2026` | `teacher@zhixueban.com` | 可进入 `/knowledge-base`，选择创建个人知识库或发布班级知识库 |
| 学生 | `student` | `Student@2026` | `student@zhixueban.com` | 只创建自己的个人知识库，可读取老师发布的班级知识库，不显示后台入口 |
